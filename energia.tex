...
En la naturaleza hay ciertas fuerzas, como la de la gravedad por ejemplo, que tienen una propiedad muy especial llamada \emph{conservativa}. Si calculamos el trabajo hecho por una fuerza al mover un objeto de un punto a otro a lo largo de una trayectoria curvada, en general el trabajo depende de la curva, pero en algunos casos especiales no. Si el trabajo no depende de la curva, decimos que la fuerza es conservativa. En otras palabras, si la integral de línea de la fuerza veces la distancia para ir de la posición 1 a 2 es calculada lo largo de una trayectoria $A$ y entonces a lo largo de $B$ y se obtiene lo mismo, y si es cierto para éste par de números a lo largo de \emph{todas las curvas}, y si pasa sin importar el par de números que usemos, entonces decimos que la fuerza es conservativa. En tales casos el trabajo se puede calcular de una manera simple, y podemos dar una fórmula para el resultado.
...
\end{align}
Generalizando el caso de la integración normal en una variable000,
...
La unidad de trabajo y energía en el SI es el joule ($\joule$):
\begin{align}
...
\begin{itemize}
\item[\textbf{Ejemplo:}] Un cuerpo de masa $m_1$ se desliza desde el
  reposo sobre una mesa sin fricción debido al peso de un bloque de
  masa $m_2>m_1$ que cae desde una altura $y_0$ y con el cual está
  conectado a través de una polea ideal. % ver figura
...
\end{align}
Como los dos bloque se mueven con la misma rapidez $v$, y asumiendo
sin perdida de generalidad que la altura final del bloque 2 es cero,
tenemos
\begin{align}
  m_2gy_0=&\tfrac{1}{2}m_1 v^2+\tfrac{1}{2}m_2 v^2\nonumber\\
  =&\tfrac{1}{2}(m_1+m_2) v^2\,,
\end{align}
despejando $v$, obtenemos
\begin{align}
  v^2=\frac{2m_2}{m_1+m_2}gy_0\,.
\end{align}
...
tenemos
\begin{align}
  a^2 \Delta t^2=\frac{1}{2}\frac{2m_2}{m_1+m_2}ga\Delta t^2\,,
\end{align}
de donde
\begin{align}
  a=\frac{2m_2}{m_1+m_2}g
\end{align}

Usando directamente las leyes de Newton:
\begin{align}
  T=&m_1 a\nonumber\\
m_2g-T=&m_2 a\,,
\end{align}
donde $T$ es la tensión de la cuerda. Despejando $a$ obtenemos
\begin{align}
...
Por sencillez consideremos el caso de un resorte que se mueve a lo largo del eje $x$. Para una elongación $x-x_0$ a partir de su posición de equilibrio $x_0$ la fuerza está dada por la Ley de Hooke
\begin{align}
...
\begin{align}
  ...
  =&\int_{x_a}^{x_b} Fdx\nonumber\\
  =&-k\int_{x_a}^{x_b} (x-x_o)dx\,.
\end{align}

De la siguiente energía potencial para el resorte:
\begin{align}
...
\begin{itemize}
\item[\textbf{Ejemplo:}] En $t=0$ una masa atada a un resorte sobre
...
\end{align}
donde hemos definido la frecuencia angula $\omega$ como:
\begin{align}
...
Para una interacción de dos partículas tenemos el resultado
\begin{align}
...
\begin{align}
  P=\frac{dW}{dt}=&\mathbf{F}\cdot \frac{\mathbf{r}}{dt}\nonumber\\
...
\end{align}
\begin{inprogress}
  Ejemplos cualitativos de potencia 
\end{inprogress}
...
\begin{align*}
  ...
  {v_1'}^2+(\tfrac{5}{6}) {v_2'}^2\nonumber\\
  \frac{11}{6}{v_1^2}'=&\frac{11}{6}v_1\nonumber\\
  {v_1^2}'=&v_1\,,
\end{align*}
y entonces
\begin{align*}
