...
\label{cha:momentum}
\begin{frame}
  La segunda ley de Newton en su forma más general puede escribirse,
  para una partícula de moméntum $\mathbf{p}$, como (ver
  ec.~(\ref{eq:28})
\begin{align}
...
\begin{frame}
  \begin{figure}
  \centering
%\only<1>{\includegraphics[scale=1.3]{centrodemasa0}}%
%\only<2>{\includegraphics[scale=1.3]{centrodemasa1}}%
%\only<3>{\includegraphics[scale=1.3]{centrodemasa2}}%
%\only<4>%
...
\ejemplo{\textbf{Centro de masa de una varilla no uniforme}.}
Calcular el centro de masa de una varilla de densidad lineal $\lambda=\lambda_0(x/L)$, donde $L$ es la longitud de la varilla
  \begin{align}
...
\end{align}
\finejemplo
%centro de masa de un cuadrado uniforme
% \textbf{Centro de masa de una hoja triangular} 
...
\ejemplo{\textbf{Retroceso de un cañón de resorte}.}
...
\begin{frame}
  \begin{figure}
  \centering
%\only<1>{\includegraphics{rocket1}}%
%\only<2>{\includegraphics{rocket1a}}%
%\only<3>%
{\includegraphics{rocket2}}
  \caption{Cohete}
  \label{fig:rocket}
\end{figure}
\end{frame}
...
Si el cohete está en presencia de un campo gravitacional constante
