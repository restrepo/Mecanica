...
\end{align}
Sustituyendo en la ec.~\ref{eq:vcurvprev}:
\begin{align}
  \label{eq:vpol}
  \mathbf{v}(t)=&
  \dot{r}(t)\hat{\mathbf{r}}(t)+r(t)\dot{\theta}(t)\hat{\boldsymbol{\theta}}(t)\,.
  \dot{r}(t)\hat{\mathbf{r}}(t)+r(t)\boldsymbol{\omega}(t)\,.
\end{align}
...
\end{align}
Para un cuerpo girando alrededor de un eje, tenemos %ver notas
\begin{align}
  \left|\frac{d\mathbf{r}}{dt}  \right|=|\mathbf{v}|=r\sin\phi\frac{d\theta}{dt}\,.
\end{align}
En forma vectorial
\begin{align}
  \mathbf{v}=&\hat{\mathbf{n}}\times\mathbf{r}\frac{d\theta}{dt}\nonumber\\
=&\frac{d\theta}{dt}\hat{\mathbf{n}}\times\mathbf{r}\nonumber\\
=&\boldsymbol{\omega}\times\mathbf{r}\,.
\end{align}
Ejemplo: rotación en un plano %ver ejemplo 7.2
\begin{align}
  \mathbf{v}(t)=&r\omega\hat{\boldsymbol{\theta}}(t)
\end{align}
...
El moméntum angular para un cuerpo rígido es
\begin{align}
  \mathbf{L}=\sum_{i=1}^N \mathbf{r}_i\times m_i\dot{\mathbf{r}}_i
\end{align}
